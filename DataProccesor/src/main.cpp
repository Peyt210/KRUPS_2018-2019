...
//cnversion constants for the sensors
#define TC_CONVERSION (.25)
#define LOACCEL_CONVERSION (16.0f/32767.5f) // +/- 2 g
#define HIACCEL_CONVERSION_MULTIPLIER (400.0F)
#define HIACCEL_CONVERSION_DIVISOR (4095.0F)
#define HIACCEL_ZERO_SHIFT (-200.0F)
#define MAG_CONVERSION_XY (((178.0f-128.0f)/(256.0f) + 1.0f) * 4912.0f / 32760.0f)  //+/- 4 gauss
#define MAG_CONVERSION_Z (((167.0f-128.0f)/(256.0f) + 1.0f) * 4912.0f / 32760.0f)
#define GYRO_CONVERSION (2000.0f/32767.5f*(3.14159265359f/180.0f)) //+/- 245 dps
...
void getLoAccelReadings(vector<uint8_t> data, size_t& loc, float* readings)
{
    for (int i = 0; i < 3; ++i)
    {
        ...
        loc += 2;
        readings[i] = float(rawData) * SENSORS_GRAVITY_STANDARD* LOACCEL_CONVERSION; //sets reading in m/s^2
    }
}
...
void getMagReadings(vector<uint8_t> data, size_t& loc, float* readings)
{
    for(int i = 0; i < 3; i++)
    {
        ...
        loc += 2;
		if(i != 2){
		readings[i] = float(rawData) * MAG_CONVERSION_XY;// * SENSORS_MILLIGAUSS_TO_MICROTESLA;
		}
		else{
		readings[i] = float(rawData) * MAG_CONVERSION_Z;// * SENSORS_MILLIGAUSS_TO_MICROTESLA;
		}
    }
}
